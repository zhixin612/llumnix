...
class PairMigrationPolicy(ABC):
    ...
    def sort_instance_infos(self, instance_infos: List[InstanceInfo], descending: bool = True) -> List[InstanceInfo]:
...
class DefragConstrained(PairMigrationPolicy):
    def pair_migration(self,
        ...
        for i in range(min(len(sorted_src_instance_infos), len(sorted_dst_instance_infos))):
            ...
            # logger.warning(f'[Zhixin] [Migration] migration pair {sorted_src_instance_infos[i].instance_id_str} -> {sorted_dst_instance_infos[i].instance_id_str}')
        return migrate_instance_pairs
